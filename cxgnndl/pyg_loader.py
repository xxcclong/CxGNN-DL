...
class PyGLoader:
    def __init__(self, config):
        ...
        self.dataset = PygNodePropPredDataset(
            name=f"ogbn-{config.dataset.name}", root=config.dataset.path.strip(config.dataset.name))
        ...
        self.train_loader = NeighborLoader(
            self.data, input_nodes=self.split_idx['train'], num_neighbors=config.sampler.train.fanouts[::-1],
            shuffle=True, batch_size=config.sampler.train.batch_size, replace=config.sampler.train.replace)
        self.train_loader.transform_fn = transform_wrapper(
            self.train_loader.transform_fn)
        self.val_loader = NeighborLoader(
            self.data, input_nodes=self.split_idx['valid'], num_neighbors=config.sampler.eval.fanouts[::-1],
            shuffle=False, batch_size=config.sampler.eval.batch_size, replace=config.sampler.eval.replace)
        self.val_loader.transform_fn = transform_wrapper(
            self.val_loader.transform_fn)
        self.test_loader = NeighborLoader(
            self.data, input_nodes=self.split_idx['test'], num_neighbors=config.sampler.eval.fanouts[::-1],
            shuffle=False, batch_size=config.sampler.eval.batch_size, replace=config.sampler.eval.replace)
